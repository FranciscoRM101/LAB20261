...
int main()
{
	...
	*/
	...
	glm::mat4 model2(1.0);//Inicializar matriz de Modelo 4x4 auxiliar para la jerarquía
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//Limpiar la ventana
		glClearColor(0.0f, 0.15f, 0.08f, 1.0f);
		...
		*/
		...
		modelaux = model;
		model = glm::scale(model, glm::vec3(1.3f, 0.2f, 1.3f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.1f, 0.1f, 0.1f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		meshList[4]->RenderMeshGeometry();

		// ARTICULACIÓN PIERNA 2 
		model = model2;
